...
clusters. The notion of similarity depends on the representation of the data and on the objective of the analysis. In many clustering methods, this relationship is expressed through a proximity measure, which may be defined as a distance measure, such as the Euclidean distance \cite{theodoridis2020machine}.
...
In this context, two representative clustering approaches are considered: K-means and Density-Based Spatial Clustering of Applications with Noise (DBSCAN). K-means is a centroid-based method that partitions data into a predefined number of groups \cite{macqueen1967kmeans,lloyd1982least}. DBSCAN is a density-based method that identifies clusters from local point density and can classify isolated points as noise \cite{ester1996dbscan}. These methods were selected because they are both simple and widely used, with K-means being a classical and well-established clustering algorithm and DBSCAN providing a commonly used density-based alternative in current clustering applications.
