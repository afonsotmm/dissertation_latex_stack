...
\hspace{\parindent}
This chapter showed that, during integrated execution, closely timed planning requests and robot
state updates could be processed in a way that produced redundant planner calls and replanning
decisions based on incomplete temporal context. These issues were associated with the sequential
processing of messages in the coordination pipeline, rather than with the TEA* planning algorithm
itself.

To mitigate these effects, batching mechanisms were introduced for both planning requests and
robot state updates. Planning request batching groups closely related requests before invoking the
Path Planner, while robot state update batching allows replanning conditions to be evaluated using
a more coherent view of the fleet state. The resulting behaviour reduces unnecessary replanning
activity and improves the temporal consistency of the complete coordination pipeline during
