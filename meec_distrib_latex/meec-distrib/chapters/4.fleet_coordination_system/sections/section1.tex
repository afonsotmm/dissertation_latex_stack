...
implemented using the TEA* algorithm~\cite{matos2025criis}, which follows a centralized and
sequential prioritized planning approach.

Planning requests include the current symbolic positions and goals of all agents. Upon
...
The centralized nature of the Path Planner can produce globally coherent solutions, particularly in
environments with shared resources or high robot density. This approach allows conflicts to be
resolved at planning time rather than during execution, which can reduce the likelihood of deadlocks,
...
to evaluate multiple robot priority orderings in parallel~\cite{ribeiro2025parallel}. This
dissertation builds on that direction in Chapters~\ref{chap:ch7} and~\ref{chap:ch8}, where the
parallel execution logic is reorganized, combined with the proposed prioritization heuristics, and
evaluated on large-scale planning scenarios.
...
navigate to a given goal vertex. When this service is called, the Navigation Handler forwards the
...
adaptive replanning while preserving centralized coordination, the Supervisor helps the fleet
maintain safe and efficient operation even when assumptions made at planning time are
