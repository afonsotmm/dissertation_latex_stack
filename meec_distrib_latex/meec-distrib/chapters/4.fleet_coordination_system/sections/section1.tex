...
illustrated in Figure~\ref{fig:criis_high_level}, the system is composed of two main parts: the
{Central Coordination Module} and the {Agent Control Module}.
...
supervisor. This separation allows each robot to retain autonomy in localization and low-level
control, while the fleet remains coordinated through centralized planning.
...
\hspace{\parindent}
The Central Coordination Module is responsible for global decision-making and coordination across
the robotic fleet. It maintains a centralized representation of the environment and robot states,
computes coordinated plans, and supervises execution~\cite{matos2025criis}. This module therefore
acts as the main coordination layer of the system, linking the graph representation, the TEA*
planner, and the execution feedback received from the robots.

As summarized in Figure~\ref{fig:criis_sequence_diagram}, this module comprises four core
components: the Graph Server, the Graph Pre-Processor, the Path Planner, and the Supervisor.
Interaction with the robots' navigation systems is mediated through a Navigation Handler, which
provides an abstraction layer for plan dispatch and execution feedback.
...
distinguished between the Central Coordination Module and the Agent Control Module. Finally, the
main components of the Central Coordination Module were described, including the Graph Server,
Graph Pre-Processor, Path Planner, Navigation Handler, and Supervisor. This architecture provides
the foundation upon which the prioritization heuristics, parallel planning framework, and batching
mechanisms developed in the following chapters are introduced.