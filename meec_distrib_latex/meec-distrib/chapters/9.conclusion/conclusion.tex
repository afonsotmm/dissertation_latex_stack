...
computing concepts. The existing CRIIS fleet coordination system was then analysed as the baseline
...
type of information used to compute robot priority scores. Their behaviour was evaluated using an
isolated Path Planner testing framework, which allowed the effect of each priority ordering to be
analysed independently from the remaining coordination stack. The experimental results showed
...
The complete CRIIS coordination stack was also analysed during integrated execution, where
...
incomplete temporal context. To mitigate this behaviour, batching mechanisms were introduced for
...
planning to robot ordering can be addressed without abandoning the existing planner architecture.
The proposed heuristics provide diverse and meaningful priority orderings, while the parallel
framework enables several of these alternatives to be explored within the same planning cycle. In
addition, the batching mechanisms improve the behaviour of the complete coordination stack during
execution. Together, these contributions provide a practical extension of the existing centralized
fleet coordination system, improving its robustness, flexibility, and suitability for larger and more
demanding multi-robot planning scenarios.